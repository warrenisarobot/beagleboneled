...
void changeBrightness(RGB *pixelIn, RGB *pixelOut, int steps, int currentStep) {
  ...
  //std::cout << "RGB(" << pixelIn->red << "," << pixelIn->green << "," << pixelIn->blue;
  if (currentStep >= 0) {
    if (currentStep > 0) {
      scaleMe = pixelIn->red;
    ...
    step = (float) difference / steps;
    intensity_scale = (scaleMe + (step * currentStep)) / scaleMe;
  } else {
    //we are dimming, this is just the ratio of the current step to total steps (if the
    //currentStep is negative steps then we are fully dimmed)
    intensity_scale = (float)(steps - (-1 * currentStep)) / steps;
  }
  ...
  //std::cout << "changeBrightness(" << currentStep << "/" << steps << ")  -> RGB(" << pixelOut->red << "," << pixelOut->green << "," << pixelOut->blue << ") step=" << step << ", intensity_scale=" << intensity_scale << " " << pixelIn->red * intensity_scale << ", steps=" << steps << ", difference=" << difference << "\r\n" ;
}
